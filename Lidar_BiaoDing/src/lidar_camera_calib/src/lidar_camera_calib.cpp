...
// 使用 Manifold 替代 LocalParameterization
      ceres::Manifold* q_manifold = new ceres::EigenQuaternionManifold();
      ceres::Problem problem;

// 添加参数块，并将 Manifold 应用到四元数参数
      problem.AddParameterBlock(ext, 4, q_manifold);  // 四元数参数
      problem.AddParameterBlock(ext + 4, 3);          // 平移参数
      ...
      if (use_vpnp) {
		...
		ceres::CostFunction* cost_function = pnp_calib::Create(val);
		problem.AddResidualBlock(cost_function, nullptr, ext, ext + 4);
	    }
       }
